...
function submitForm(e) {
    ...
    if (regex.test(firstNameInput.value) === false) {
    ...
    const regex =
        /^[a-zA-ZéèîïÉÈÎÏ][a-zéèêàçîï]+([-'\s][a-zA-ZéèîïÉÈÎÏ][a-zéèêàçîï]+)?$/;
    if (regex.test(lastNameInput.value) === false) {
    ...
    if (regex.test(address) === false) {
        alert("Veuillez entrer une valid adress");
        return 
    }
    ...
    if (regex.test(city) === false) {
        alert("Veuillez entrer une valid ville");
        return 
    }
}
https://github.com/EtudianteHU/Uyarci_Hilal_code_5_042023.git